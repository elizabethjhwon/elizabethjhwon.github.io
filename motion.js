(()=>{
  ...
  function storyFish(px,py,size,dir,body,tail,t,index){const s=size*Math.max(.72,Math.min(1.08,width/1250));ctx.save();ctx.translate(px,py+Math.sin(t*.9+index)*5);ctx.scale(dir,1);ctx.lineWidth=1.35;ctx.strokeStyle='#dce9f2';ctx.lineJoin='round';
    ctx.fillStyle=tail+'aa';ctx.beginPath();ctx.moveTo(-38*s,0);ctx.lineTo(-64*s,-27*s);ctx.quadraticCurveTo(-55*s,0,-65*s,29*s);ctx.lineTo(-37*s,10*s);ctx.closePath();ctx.fill();ctx.stroke();
    ...
    ctx.fillStyle=body;ctx.beginPath();ctx.moveTo(-3*s,-28*s);ctx.lineTo(5*s,-48*s);ctx.lineTo(15*s,-28*s);ctx.closePath();ctx.fill();ctx.stroke();ctx.beginPath();ctx.moveTo(2*s,27*s);ctx.lineTo(10*s,43*s);ctx.lineTo(20*s,25*s);ctx.closePath();ctx.fill();ctx.stroke();
    ...
    ctx.restore()}
  function draw(ms){const t=reduce?0:ms/1000,g=ctx.createLinearGradient(0,0,0,height*.52);g.addColorStop(0,'#031127');g.addColorStop(.7,'#08264a');g.addColorStop(1,'#164a70');ctx.fillStyle=g;ctx.fillRect(0,0,width,height);for(let i=0;i<28;i++){const seed=(i*47)%101,alpha=.14+.2*Math.sin(t*.45+i);ctx.fillStyle=`rgba(225,239,255,${alpha})`;ctx.beginPath();ctx.arc(width*(seed/101),height*(.06+((i*31)%41)/100),i%5===0?1.3:.7,0,7);ctx.fill()}moon();water(t);fish.forEach((f,i)=>{const travel=width+180,raw=(f.start*travel+t*f.speed)%travel,px=f.dir>0?raw-90:width+90-raw;storyFish(px,height*f.y,f.size,f.dir,f.body,f.tail,t,i)});ctx.fillStyle='#f6f2e8';ctx.textBaseline='middle';ctx.font=`400 ${Math.max(30,Math.min(58,width*.047))}px Newsreader,Georgia,serif`;ctx.fillText('elizabeth won',width<600?width*.08:width*.1,height*.16);ctx.font='11px IBM Plex Mono,monospace';ctx.fillStyle='#8eb8ff';ctx.fillText('MATHEMATICS  ·  LAW  ·  LANGUAGE',width<600?width*.08:width*.1,height*.22);if(!reduce)requestAnimationFrame(draw)}
