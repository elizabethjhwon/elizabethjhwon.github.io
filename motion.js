(()=>{
  ...
  function storyFish(px,py,size,dir,body,tail,t,index){const s=size*.67*Math.max(.72,Math.min(1.08,width/1250));ctx.save();ctx.translate(px,py+Math.sin(t*.9+index)*5);ctx.scale(dir,1);ctx.lineWidth=1.15;ctx.strokeStyle='#dce9f2';ctx.lineJoin='round';
    const flutter=Math.sin(t*4.2+index*1.7)*.3;ctx.save();ctx.translate(-30*s,0);ctx.rotate(flutter);ctx.fillStyle=tail+'66';for(let petal=0;petal<3;petal++){const angle=(-.7+petal*.7)+Math.sin(t*3+petal+index)*.04;ctx.save();ctx.rotate(angle);ctx.beginPath();ctx.moveTo(0,0);ctx.bezierCurveTo(-12*s,-3*s,-26*s,-8*s,-32*s,0);ctx.bezierCurveTo(-25*s,9*s,-11*s,7*s,0,2*s);ctx.closePath();ctx.fill();ctx.stroke();ctx.restore()}ctx.restore();
    ...
    const fin=Math.sin(t*3.4+index)*.16;ctx.fillStyle=tail+'55';ctx.save();ctx.translate(0,-8*s);ctx.rotate(fin);ctx.beginPath();ctx.moveTo(-4*s,0);ctx.quadraticCurveTo(0,-12*s,7*s,0);ctx.closePath();ctx.fill();ctx.stroke();ctx.restore();
    ctx.fillStyle='#f3f0e7';ctx.beginPath();ctx.arc(28*s,-3*s,3.8*s,0,Math.PI*2);ctx.fill();ctx.stroke();ctx.fillStyle='#071d3e';ctx.beginPath();ctx.arc(29*s,-3*s,1.35*s,0,Math.PI*2);ctx.fill();
    ctx.beginPath();ctx.moveTo(35*s,3*s);ctx.quadraticCurveTo(39*s,5*s,42*s,3*s);ctx.stroke();ctx.restore()}
